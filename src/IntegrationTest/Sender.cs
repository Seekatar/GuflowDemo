...
namespace IntegrationTest
{
    ...
    /// </summary>
    public class TestWorkflow3SavedConsumerDefinition : ConsumerDefinition<IntegrationTestWorkflow3SavedConsumer>
    {
        public TestWorkflow3SavedConsumerDefinition()
        {
    ...
    public class IntegrationTestWorkflow3SavedConsumer : IConsumer<IWorkflow3Saved>
    {
        static int i = 1;
        ...
        public async Task Consume(ConsumeContext<IWorkflow3Saved> context)
        {
            _event.Set();
            await Out.WriteLineAsync($">>> {nameof(IntegrationTestWorkflow3SavedConsumer)} {i++} - saved: Name = {context.Message.Name} Id: {context.Message.Id}");
    ...
    /// </summary>
    /// <remarks>must be public to autoregister by convention, and name should be unique to avoid colliding with other consumers</remarks>
    public class IntegrationTestWorkflow3DeletedConsumer : IConsumer<IWorkflow3Deleted>
    {
        static readonly AutoResetEvent _event = new(false);
        ...
        public async Task Consume(ConsumeContext<IWorkflow3Deleted> context)
        {
            _event.Set();
            await Out.WriteLineAsync($">>> {nameof(IntegrationTestWorkflow3DeletedConsumer)} - deleted");
        }
    }
    ...
    public class Sender : IDisposable
    {
        ...
        public async Task<Workflow3?> SaveWorkflow3(ISaveWorkflow3 message, CallerIdentity identity)
        {
            return (await _busControl.RequestResponse<ISaveWorkflow3, ISaveWorkflow3Response>(message, identity).ConfigureAwait(false))?.Workflow3;
        }
        ...
        {
            var sendEndpoint = await AwaitNullable(_busControl?.GetPublishSendEndpoint<IWorkflow3Saved>());
            if (sendEndpoint != null)
            {
                await sendEndpoint.Send(new Workflow3Saved
                {
        ...
        public async Task<Workflow3?> GetWorkflow3(IGetWorkflow3 getIt, CallerIdentity identity)
        {
            return (await _busControl.RequestResponse<IGetWorkflow3, IGetWorkflow3Response>(getIt, identity).ConfigureAwait(false))?.Workflow3;
        }
        ...
        public async Task DeleteWorkflow3(DeleteWorkflow3 message, CallerIdentity identity)
        {
            var sendEndpoint = await AwaitNullable(_busControl?.GetSendEndpoint(IDeleteWorkflow3.QueueUri));
            if (sendEndpoint != null)
            {
