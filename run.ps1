...
.PARAMETER DockerTag
Optional tag for the docker container, leave blank for ccc-cas-workflow3-service:MM.dd.yy
...
Version number for the Message package
...
[CmdletBinding()]
param(
    [ValidateSet('default','ci','UpdateRunPs1','DumpVars','DotnetBuild','StartBack','StopBack','TestClient','GetReady','IntegrationTest','UnitTest','OpenSln','BuildMessage','DockerBuild','HelmDependencyBuild','HelmInstall','HelmUninstall','DockerRun','DockerInteractive','DockerStop')]
    [string[]] $Task = 'Default',
    [string] $DockerEnvFile = (Join-Path $PSScriptRoot "env.list"),
    [string] $ContainerName = "workflow3-service",
    [string] $DockerTag = "ccc-cas-workflow3-service:$((Get-Date).ToString('MM.dd.yy'))",
...
Invoke-Psake -buildFile '.\psakefile.ps1' -taskList @($Task) -parameters @{
    ...
    runFolder = 'CCC.CAS.Workflow3Service'
